...
+ в hosts-файле ip-адрес машины должен быть сопоставлен сначала с полным FQDN-именем и только затем с кратким. FreeIPA требует, чтобы hostname возвращал именно FQDN 
...
```
1.3 Запускаем плэйбук deploy_ipa_clients.yml 
<details>
...
Примечание: если выдается "Неизвестная ошибка" при входе, нужно очистить кэш. Браузеры очень цепко держат старые сессии и самоподписанные сертификаты. Или делать первый вход в режиме Инкогнито.
